...
def answer(puzzle_number, actual, expected):
    """ Verify if the answer we got was the correct answer. """
    assert actual == expected, f'For {puzzle_number}, expected {expected} but got {actual}.'
    return True

def parse(day, parser=str, sep='\n', print_lines=7) -> tuple:
    """ Split the day's input file into entries separated by `sep`, and apply
        `parser` to each.
    """
...
def words(text: str) -> List[str]:
    """ A list of the alphabetic words in text, ignoring non-letters."""
...
class Grid(dict):
    """ A 2D grid, implemented as a mspping of {(x, y): cell_contents}. """
    def __init__(self, mapping=(), rows=(), neighbors=neighbors4):
        """Initialize with e.g., either `mapping={(0, 0): (1, 0): 2, ...}`,
        or rows=[(1,2,3), (4,5,6)].
